# Creating the AoU VDS

The following example walks through the procedure we followed for creating the Delta VDS in November 2022. This will use the `gvs_import.py` script created by the Hail team. 
...
## Locate the avro files that will be used to create the VDS
(If you have not create avro files, please run the wdl GvsExtractAvroFilesForHail)


```python3 hail_gvs_import.py  
--avro_prefix  gs://<firecloud-bucket-id>/submissions/<workflow-id>/GvsExtractAvroFilesForHail/<submission-id>/call-OutputPath/avro/  
--write_prefix gs://<firecloud-bucket-id>/submissions/<workflow-id>/GvsExtractAvroFilesForHail/<submission-id>/call-OutputPath/<where the VDS should be written>/
```

Within the Python REPL created in the preceding steps, read the hail script:
