...
public class ClosestSVFinder {
    ...
    public Map.Entry<Long, SVCallRecord> getClosestItem(final SVCallRecord evalRecord, final Set<Map.Entry<Long, SVCallRecord>> candidates) {
        ...
        // For stability, in case all other criteria are equal
        final Comparator<Map.Entry<Long, SVCallRecord>> idComparator = Comparator.comparing(o -> o.getValue().getId());
        final Optional<Map.Entry<Long, SVCallRecord>> result = candidates.stream()
                ...
                .min(distanceComparator.thenComparing(minDistanceComparator).thenComparing(genotypeDistanceComparator).thenComparing(idComparator));
    ...
    private static class ActiveClosestPair {
        ...
        boolean update(final Long newClosestId, final SVCallRecord newClosest) {
            ...
            final Map.Entry<Long, SVCallRecord> result = candidates.stream()
                    .min(distanceComparator.thenComparing(minDistanceComparator).thenComparing(genotypeDistanceComparator).thenComparing(idComparator)).get();
