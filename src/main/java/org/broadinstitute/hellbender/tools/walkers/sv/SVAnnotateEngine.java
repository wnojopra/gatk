    ...
    protected static GATKSVVCFConstants.StructuralVariantAnnotationType getSVType(final VariantContext variant) {
        ...
        if (alt.isBreakpoint()) {
            ...
            if (alt.toString().contains("INS")) {
                ...
                return GATKSVVCFConstants.StructuralVariantAnnotationType.INS;
            } else {
                // parse ALT as symbolic allele, assuming format <SVTYPE>
                return GATKSVVCFConstants.StructuralVariantAnnotationType.valueOf(alt.toString().substring(1, alt.toString().length()-1));
            }
        } else {
